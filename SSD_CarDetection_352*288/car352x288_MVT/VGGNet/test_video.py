...
model_weight = "VGG_car352x288_SSD_352x288_iter_50000.caffemodel"
...
while(True):
    ...
    for i in range(max(0, top_conf.shape[0])):
        ...
        #print "conf:" + str(top_conf[0])
        #print "det_label:" + str(det_label[0])
        cv2.rectangle(image, (xmin,ymin),(xmax,ymax),(255,0,0),5)
        image = cv2.putText(image, str(top_label[i]), (xmin,ymin), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 255, 255), 2)
        image = cv2.putText(image, str(top_conf[i]), ((xmax + xmin)/2,(ymax+ymin)/2), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 1)
#    print image_data.shape
